    ...
    <div class="products-grid" id="flowersGrid">
        @foreach($flowers as $flower)
            <div class="product-card" data-name="{{ strtolower($flower['name']) }}">
                <button class="flower-link" type="button" data-image="{{ asset('images/flowers/'.$flower['image']) }}" data-title="{{ $flower['name'] }}">
                    <div class="product-image">
                        <img class="flower-image" src="{{ asset('images/flowers/'.$flower['image']) }}" alt="{{ $flower['name'] }}">
                    </div>
                </button>
...
<script>
    ...
    function renderPage() {
        ...
        flowersToDisplay.forEach(flower => {
            ...
            card.innerHTML = `
                <button class="flower-link" type="button" data-image="${flower.image ? '/images/flowers/' + flower.image : ''}" data-title="${flower.name}">
                    <div class="product-image">
                        <img class="flower-image" src="${flower.image ? '/images/flowers/' + flower.image : ''}" alt="${flower.name}">
                    </div>
                </button>
                <div class="product-info">
                    <h3>${flower.name}</h3>
